  ...
  <AdminLayout>
    ...
    <div class="bg-white rounded-2xl p-4 shadow-soft border border-gray-100 mb-6">
      <div class="flex items-center gap-2 overflow-x-auto">
        ...
        >
          {{ tab.name }}
          <span v-if="tab.count" class="ml-2 bg-slate-200 text-slate-700 text-xs px-2 py-0.5 rounded-full">
            {{ tab.count }}
          </span>
        </button>
      </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full">
          ...
          <tbody class="divide-y divide-gray-100">
            <tr v-for="request in filteredRequests" :key="request.id" class="hover:bg-beige/50 transition-colors">
              <td class="p-4">
              ...
              <td class="p-4">
                <span
                  :class="[
                    ...
                    request.status === 'Validée' ? 'bg-teal-100 text-teal-700' :
              ...
              <td class="p-4 text-right">
                <button class="text-slate-500 hover:text-navy-900 transition-colors">
                  Voir
                </button>
              </td>
            </tr>
...
const filteredRequests = computed(() => {
  ...
  if (activeTab.value === 'revision') return requests.value.filter(r => r.status === 'En révision')
