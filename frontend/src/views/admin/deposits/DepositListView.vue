...
function toggleSelect(id) {
  ...
  isSelectAll.value =
    selectedIds.value.length === paginatedDeposits.value.length &&
    paginatedDeposits.value.length > 0
}
...
async function confirmAssign() {
  ...
  try {
    ...
    if (isSecondOpinion.value) {
      await store.updateDepositStatus(assignTargetId.value, 'second_opinion', {
...
async function confirmBulkReject() {
  ...
  try {
    ...
    if (skippedCount > 0)
      toast.warning(`${skippedCount} demande(s) ignorée(s) (statut non compatible).`)
  } catch {
...
<template>
  <AdminLayout>
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex items-center gap-3 flex-wrap">
          ...
          >
            ...
            <span
              ...
              class="bg-teal-600 text-white text-xs px-1.5 py-0.5 rounded-full"
              >!</span
            >
          </button>
          <button
      ...
      <div
        v-if="showFilters"
        class="bg-white rounded-2xl p-5 border border-gray-100 shadow-soft mb-4"
      >
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div>
            ...
            <select
              ...
              class="w-full bg-beige border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-teal-500"
            >
      ...
      <div class="bg-white rounded-2xl shadow-soft border border-gray-100 overflow-hidden">
        <div
          ...
          class="flex items-center gap-3 px-4 py-3 bg-navy-50 border-b border-navy-100 flex-wrap"
        >
          <CheckSquare class="w-4 h-4 text-navy-600 shrink-0" />
          <span class="text-sm font-medium text-navy-700"
            >{{ selectedIds.length }} sélectionnée(s)</span
          >
          ...
          <table class="w-full min-w-225">
            <thead>
              <tr class="border-b border-gray-200 bg-gray-50/50">
                ...
                <th
                  class="text-left p-4 text-xs font-semibold text-slate-500 uppercase tracking-wider whitespace-nowrap"
                >
                  Réf.
                </th>
                <th
                ...
                <th
                  class="text-left p-4 text-xs font-semibold text-slate-500 uppercase tracking-wider cursor-pointer select-none whitespace-nowrap"
                ...
                <th
                  class="text-right p-4 text-xs font-semibold text-slate-500 uppercase tracking-wider whitespace-nowrap"
                >
                  Actions
                </th>
              </tr>
            ...
            <tbody class="divide-y divide-gray-100">
              <template v-for="deposit in paginatedDeposits" :key="deposit.id">
                <tr
                  ...
                  :class="{
                    'bg-teal-50/30': selectedIds.includes(deposit.id) || isExpanded(deposit.id),
                  }"
                ...
                >
                  ...
                  <td class="p-4">
                    <span class="text-sm font-mono text-slate-600 whitespace-nowrap"
                      >DEP-{{ deposit.id }}</span
                    >
                  </td>
                  <td class="p-4">
                    <div class="flex items-center gap-3 max-w-xs">
                      <div
                        class="w-10 h-10 bg-navy-50 rounded-xl flex items-center justify-center shrink-0"
                      >
                        <FileText class="w-5 h-5 text-navy-400" />
                      </div>
                      <div class="min-w-0">
                        <p class="font-medium text-[#1B2A4A] text-sm truncate">
                          {{ deposit.title }}
                        </p>
                  ...
                  <td class="p-4">
                    <div class="flex items-center gap-2">
                      <div
                        class="w-8 h-8 bg-teal-100 rounded-full flex items-center justify-center text-teal-700 text-xs font-bold shrink-0"
                      >
                        {{ store.getUserInitials(deposit.submittedBy) }}
                      </div>
                      <span class="text-sm text-[#1B2A4A] whitespace-nowrap"
                        >{{ deposit.submittedBy?.first_name }}
                        {{ deposit.submittedBy?.last_name }}</span
                      >
                    </div>
                  </td>
                  <td class="p-4">
                  ...
                  <td class="p-4 text-right" @click.stop>
                    <div class="flex items-center justify-end gap-1">
                      ...
                      >
                        <component
                          :is="isExpanded(deposit.id) ? ChevronUp : ChevronDown"
                          class="w-4 h-4"
                        />
                      </button>
                    </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 p-6">
                      <div>
                        ...
                        <div class="flex gap-4">
                          <div
                            class="w-20 h-24 bg-gray-100 rounded-lg flex items-center justify-center shrink-0"
                          >
                            <FileText class="w-8 h-8 text-navy-300" />
                          </div>
                          <div class="min-w-0">
                            <p class="font-serif font-bold text-navy-800 mb-1">
                              {{ deposit.title }}
                            </p>
                            <p class="text-sm text-gray-600">
                              Déposant : {{ deposit.submittedBy?.first_name }}
                              {{ deposit.submittedBy?.last_name }}
                            </p>
                            <p class="text-sm text-gray-600">
                              Type : {{ store.getTypeLabel(deposit.type) }}
                            </p>
                            <p class="text-sm text-gray-600 mb-2">
                              Catégorie : {{ deposit.category?.name || 'Non spécifiée' }}
                            </p>
                            <a
                              v-if="deposit.file"
                              href="#"
                              @click.prevent
                              class="text-teal-600 text-sm font-medium hover:underline inline-flex items-center gap-1"
                            >
                        ...
                        <div class="mt-4">
                          <h4 class="text-xs font-bold text-slate-500 uppercase tracking-wide mb-2">
                            Historique
                          </h4>
                          <div class="space-y-1.5">
                            <div
                              v-for="(entry, i) in deposit.history"
                              :key="i"
                              class="flex items-start gap-2 text-sm"
                            >
                              <div
                                class="w-1.5 h-1.5 mt-1.5 rounded-full bg-navy-400 shrink-0"
                              ></div>
                              <div>
                                <span class="text-navy-800">{{ entry.action }}</span>
                                <span class="text-gray-400">
                                  — {{ entry.actor }} · {{ store.formatDate(entry.at) }}</span
                                >
                                <p v-if="entry.comment" class="text-gray-500 italic">
                                  « {{ entry.comment }} »
                                </p>
                              </div>
                            </div>
                      ...
                      <div>
                        <h3
                          class="text-xs font-bold text-slate-500 uppercase tracking-wide mb-3 flex items-center gap-1.5"
                        >
                        ...
                        <div
                          ...
                          class="bg-white border border-gray-200 rounded-xl px-4 py-3 mb-3 text-sm text-gray-500 flex items-center gap-2"
                        >
                          <FileText class="w-4 h-4 shrink-0" /> Cette demande n'est pas encore
                          assignée.
                        </div>
                        <div
                          v-else-if="deposit.status === 'assigned'"
                          class="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 mb-3 text-sm text-blue-700 flex items-center gap-2"
                        >
                          <Clock class="w-4 h-4 shrink-0" /> Assignée à
                          {{ getAssigneeName(deposit) }} — en attente de revue.
                        </div>
                        <div
                          v-else-if="deposit.status === 'second_opinion'"
                          class="bg-purple-50 border border-purple-100 rounded-xl px-4 py-3 mb-3 text-sm text-purple-700 flex items-center gap-2"
                        >
                          <Eye class="w-4 h-4 shrink-0" /> En attente du second avis.
                        </div>
                        <div
                          v-else-if="deposit.status === 'rejected'"
                          class="bg-gray-100 border border-gray-200 rounded-xl px-4 py-3 mb-3 text-sm text-gray-500"
                        >
                        ...
                        <button
                          @click="viewDetail(deposit.id)"
                          class="mt-3 text-teal-600 text-sm font-medium hover:underline"
                        >
              ...
              <tr v-if="!paginatedDeposits.length">
                <td colspan="8" class="text-center py-16 text-slate-400 text-sm">
                  ...
                  <button
                    v-if="hasActiveFilters"
                    @click="clearFilters"
                    class="block mx-auto mt-2 text-teal-600 font-medium hover:underline"
                  >
                    Effacer les filtres
                  </button>
                </td>
        ...
        <div
          class="flex items-center justify-between px-6 py-4 border-t border-gray-100 flex-wrap gap-4"
        >
          <div class="flex items-center gap-2">
      ...
      <Teleport to="body">
          ...
          <div class="bg-white rounded-2xl p-6 w-full max-w-md mx-4">
            <div class="flex items-center justify-between mb-4">
              <h3 class="text-lg font-semibold text-navy-800">
                {{ isSecondOpinion ? 'Demander un 2ème avis' : 'Assigner à un responsable' }}
              </h3>
            ...
            <p class="text-sm text-gray-500 mb-4">
              Sélectionnez un responsable disponible (sans demande en cours d'examen).
            </p>
            <select
            ...
            <div v-if="isSecondOpinion" class="space-y-3 mb-4">
              ...
              <div class="flex items-center justify-between text-sm">
                <span :class="secondOpinionComment.length < 20 ? 'text-red-600' : 'text-green-600'"
        ...
        <div
          ...
          class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4"
          @click.self="isActionSubmitting ? null : (showActionDialog = false)"
        >
          <div class="bg-white rounded-2xl p-6 w-full max-w-lg mx-4">
            ...
            <p class="text-sm text-gray-600 mb-4">
              Êtes-vous sûr de vouloir {{ actionDef?.label?.toLowerCase() }} ?
            </p>
            <div v-if="actionDef?.requiresComment" class="space-y-3">
              ...
              <textarea
                ...
                :class="
                  actionComment.length < actionDef.minLength
              ...
              <div class="flex items-center justify-between text-sm">
                <span
                  :class="
                    actionComment.length < actionDef.minLength ? 'text-red-600' : 'text-green-600'
                  "
                  >{{ actionComment.length }} / {{ actionDef.minLength }}</span
                >
              </div>
            </div>
            <div class="flex gap-3 justify-end mt-6">
              <button
                ...
                class="px-4 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-600 hover:bg-gray-50 disabled:opacity-50"
              >
                Annuler
              </button>
              <button
        ...
        <div
          v-if="showBulkAssignDialog"
          class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4"
          @click.self="showBulkAssignDialog = false"
        >
          <div class="bg-white rounded-2xl p-6 w-full max-w-md mx-4">
            <div class="flex items-center justify-between mb-4">
            ...
            <p class="text-sm text-gray-500 mb-4">
              {{ selectedIds.length }} demande(s) sélectionnée(s)
            </p>
            <select
        ...
        <div
          v-if="showBulkRejectDialog"
          class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4"
          @click.self="showBulkRejectDialog = false"
        >
          <div class="bg-white rounded-2xl p-6 w-full max-w-lg mx-4">
            <div class="flex items-center justify-between mb-4">
