  ...
  <ResponsableLayout>
    ...
    <div class="bg-teal-50 border border-teal-200 rounded-xl p-4 mb-6 flex items-center gap-3">
      ...
      <p class="text-sm text-teal-800">
        Votre rôle : Examiner les demandes de dépôt qui vous sont assignées et émettre un avis motivé.
      </p>
    </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-2xl border border-gray-100 overflow-hidden">
          <div class="px-6 py-4 border-b border-gray-50 flex justify-between items-center">
            <h2 class="text-sm font-bold text-[#1B2A4A] uppercase tracking-wide">Demandes à traiter en priorité</h2>
            <router-link to="/manager/deposits" class="text-xs text-teal-600 hover:underline">Voir tout</router-link>
          </div>
            ...
            <table class="w-full text-sm">
              <thead class="bg-[#F8F7F4]">
                <tr>
                  ...
                  <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Déposant</th>
                  <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider hidden md:table-cell">Type</th>
                  <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Retard</th>
              ...
              <tbody class="divide-y divide-gray-50">
                ...
                <tr v-for="deposit in priorityRequests" :key="deposit.id" class="hover:bg-[#F8F7F4] transition-colors">
                  <td class="px-5 py-3.5 font-medium text-[#1B2A4A] truncate max-w-[200px]" :title="deposit.title">
                    {{ deposit.title }}
                  </td>
                  ...
                  <td class="px-4 py-3.5">
                    ...
                    >
                      <AlertTriangle class="w-3 h-3" /> {{ store.getAgingBadge(deposit.submittedAt).label }}
                    </span>
                    <span v-else class="text-gray-400 text-xs">—</span>
                  </td>
                  <td class="px-4 py-3.5">
                    <router-link :to="`/manager/deposits/${deposit.id}/review`" class="flex items-center gap-1 text-[#0D9488] text-sm font-medium hover:underline whitespace-nowrap">
        ...
        <div class="space-y-5">
          ...
          <div class="bg-white rounded-2xl border border-gray-100 p-5">
            <h3 class="text-sm font-bold text-[#1B2A4A] uppercase tracking-wide mb-4">Traitées récemment</h3>
            <div class="space-y-3">
              <div v-if="recentDecisions.length === 0" class="text-sm text-gray-500 py-2">
                Aucune demande traitée récemment.
              </div>
              <div v-for="deposit in recentDecisions" :key="deposit.id" class="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
          ...
          <!-- Notifications -->
          <div class="bg-white rounded-2xl border border-gray-100 p-5">
            <h3 class="text-sm font-bold text-[#1B2A4A] uppercase tracking-wide mb-4">Notifications</h3>
            <div class="space-y-3 text-sm">
              <div v-if="priorityRequests.length > 0" class="p-3 bg-amber-50 rounded-lg border-l-4 border-amber-400 text-amber-800">
...
import { Info, ClipboardList, Clock, CheckCircle, XCircle, AlertTriangle, ArrowRight } from '@lucide/vue'
...
const treatedCount = computed(() => store.deposits.filter(d => ['manager_approved', 'manager_rejected', 'approved_published', 'rejected'].includes(d.status)).length)
...
const recentDecisions = computed(() => {
  return store.deposits
    .filter(d => ['manager_approved', 'manager_rejected', 'approved_published', 'rejected'].includes(d.status))
