<template>
  <Teleport to="body">
    <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div
        class="relative bg-white rounded-3xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden animate-scale-in"
      >
        <div class="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
          <div>
            <h3 class="text-lg font-bold text-gray-900">Nouveau rendez-vous</h3>
            <p class="text-sm text-gray-500 mt-0.5">
              {{ formatDate(selectedDate) }} · {{ formatTime(slot?.start) }} -
              {{ formatTime(slot?.end) }}
            </p>
          </div>
          <button
...
const canSubmit = computed(
  () => form.value.firstName && form.value.lastName && form.value.email && props.bookingSlot,
)

watch(
  () => [props.isOpen, authStore.user],
  () => {
